...
sub extract_content {
    ...
    while ($content =~ s/Mrs\.\sClinton\s(said|added|claimed|stated|asked|exclaimed)[\s\w]*(\:|\,).\“([\-\’\.\,\?\!\s\w]+)\”//) {
        $words = $3;
    ...
    while ($content =~ s/\“([\-\’\.\,\?\!\s\w]+)\”[\s\w\,]*(said|added|claimed|stated|asked|exclaimed)[\s\w]*Mrs\.\sClinton//) {
        $words = $1;
        last if defined($output{$words});        
        # print $trump "\n";
        print "$words \n";
    ...
    while ($content =~ s/\“([\-\’\.\,\?\!\s\w]+)\”[\s\w]*Mrs\.\sClinton[\s\w]*(said|added|claimed|stated|asked|exclaimed)//) {
        $words = $1;
        last if defined($output{$words});        
        # print $trump "\n";
        print "$words \n";
